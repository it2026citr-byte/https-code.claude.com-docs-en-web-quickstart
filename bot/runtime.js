...
export const PARAMS = {
  ...
  // Выключен по вердикту 29.08: после исправления сдвига перемерен
  // честно — хуже случайного входа (−0.171R). СТРАТЕГИИ.md, раздел 5а.
  leadlag_on: { g: "отстающие", def: () => 0, opts: [1, 0],
                title: "Сканер",             unit: "" },
  ...
  // Микроструктура: не входить, когда весь ход часа сделан в последние
  // 15 минут — это кульминация, а не начало движения. Замер в gate.js.
  f_micro:    { g: "отбор", def: () => 1, opts: [0, 1],
                title: "Без финального выброса", unit: "" },
...
export const fmtVal = (key) => {
  ...
  if (key === "f_btc" || key === "f_agree" || key === "f_trend" || key === "f_figure" || key === "f_micro")
    return v === 1 ? "да" : "нет";
