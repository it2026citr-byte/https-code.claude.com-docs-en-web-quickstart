  ...
  (async function reportLoop() {
    for (;;) {
      try {
        ...
        // Отметка ставится ДО отправки, при ошибке снимается: брошенная
        // сборка повторится через минуту, а смерть процесса после
        // рассылки не даст дубля. Остаточный риск — смерть ВНУТРИ
        // рассылки: тогда день потерян. Выбор сознательный: дубль
        // сводки хуже, потому что неотличим от задвоившегося бота.
